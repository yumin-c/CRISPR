...
class BalancedMSELoss_rgn(nn.Module):

    def __init__(self, scale=True):
        super(BalancedMSELoss_rgn, self).__init__()

        self.factor = [1, 0.6, 0.5]
...
class ScaledMSELoss_rgn(nn.Module):

    def __init__(self):
        super(ScaledMSELoss_rgn, self).__init__()

    def forward(self, pred, y):
        mu = 1 / (torch.exp(-10 * y + 2.5) + torch.ones_like(y)) # 0220 loss
...
class PE7MSELoss(nn.Module):

    def __init__(self):
        super(PE7MSELoss, self).__init__()

        self.factor = [1.0, 1.5, 1.6] # Reciprocal of sqrt class ratio
        print("Applying ScaledMSELoss")

    def _scale_mseloss(self, pred, y):
        mu = 1 / (torch.exp(-10 * y + 2.5) + torch.ones_like(y)) # 0220 loss

        return torch.sum(mu * (y-pred)**2) # Note that we are not dividing the result by the number of samples at this point
    
    def forward(self, pred, actual):
        pred = pred.view(-1, 1)
        y = torch.log1p(actual[:, 0].view(-1, 1))

        l1 = self._scale_mseloss(pred[actual[:, 1] == 1], y[actual[:, 1] == 1]) * self.factor[0]
        l2 = self._scale_mseloss(pred[actual[:, 2] == 1], y[actual[:, 2] == 1]) * self.factor[1]
        l3 = self._scale_mseloss(pred[actual[:, 3] == 1], y[actual[:, 3] == 1]) * self.factor[2]

        return (l1 + l2 + l3) / pred.size(0) # Division to calculate class-balanced, scaled mean squared error